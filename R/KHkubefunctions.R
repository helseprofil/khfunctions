  ...
  for (del in names(KB)) {
    ...
    if (del %in% names(DesTIL$Part) & del %in% names(DesFRA$Part)) {
      ...
      if (globs$DefDesign$DelType[del] == "COL") {
        ...
        # print("aksdloekasloedkoealsdkoekoe")
        # print(IntFra)
        # print(IntTil)
...
FinnKodebokIntervaller <- function(FRA, TIL, storst = TRUE, delnavn = "INT") {
  ...
  if (storst == TRUE) {
    # Gir hoeyest prioritet til aa bruke/beholde store intervaller
    sorter <- order(intervals::size(FRAi), decreasing = TRUE)
  } else {
    # Gir hoeyest prioritet til aa bruke smaa intervaller og aggregger opp. Brukes ved aldersstandardisering
