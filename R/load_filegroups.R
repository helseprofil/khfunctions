...
load_and_format_filegroups <- function(parameters){
  ...
  for (fil in unique(parameters$filer)) {
    tabfilter <- ifelse(fil == parameters$filer$TELLER, parameters$tabfilter, "")
    
    
    # hent filgruppeparametre
    # 
    lesfil
    # 
    
    
    FILinfo <- KlargjorFil(filename = fil, tabfilter = tabfilter, versjonert = versjonert, batchdate = batchdate, filefilter = parameters$FILFILTRE, globs = globs)
