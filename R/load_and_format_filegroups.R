...
load_and_format_files <- function(parameters){
  ...
  for (file in nonteller_files) {
    ...
    load_filegroup_to_buffer(filegroup = file, filter = nontellerfile_filter, parameters = parameters)
  }
  invisible(gc())
}
...
#' @noRd
load_filegroup_to_buffer <- function(filegroup, filter, parameters){
  ...
  if(isfilefilter){
    ...
    Filter <- SettFilterDesign(filefilter, bruk0 = FALSE, FGP = fileinfo, parameters = parameters)
    # TODO: optimalisere OmkodFil
    if (length(Filter) > 0) FIL <- OmkodFil(FIL, FinnRedesign(FinnDesign(FIL, parameters = parameters), list(Parts = Filter), parameters = parameters), parameters = parameters)
    ...
    if(isrsynt1) {
      # do_special_handling(dt = FIL, code = filefilter$FF_RSYNT1, parameters = parameters)
...
do_harmonize_geo <- function(file, vals = list(), rectangularize = TRUE, parameters) {
  ...
  if(rectangularize){
    ...
    design <- FinnDesign(file, parameters = parameters)
    
    for (Gn in design$Part[["Gn"]][["GEOniv"]]) {
      validgeo <- data.table::data.table(GEO = parameters$GeoKoder[GEOniv == Gn & FRA <= parameters$khyear & TIL > parameters$khyear]$GEO)
