...
load_filegroup_to_duckdb <- function(con, filegroup, parameters){
  invisible(DBI::dbExecute(con, sprintf("DROP TABLE IF EXISTS %s", 
                                        as.character(DBI::dbQuoteIdentifier(con, filegroup)))))
  ...
  orgfile <- if(isfilefilter){ filefilter$ORGFIL } else { filegroup}
  ...
  allcols <- DBI::dbGetQuery(con, paste0("DESCRIBE SELECT * FROM read_parquet(", 
                                         DBI::dbQuoteString(con, orgfilepath),")"))$column_name
...
identify_readcols <- function(allcols, bef = FALSE,  parameters){
  ...
  if(!bef){
    cols <- setdiff(allcols, c("KOBLID", "ROW"))
  } else {
    befcols <- grep("^BEF|^mBEF", unlist(parameters$TNPinformation[c("TELLERKOL", "NEVNERKOL")],
