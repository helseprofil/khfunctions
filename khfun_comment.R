...
globglobs<-list(
  ...
  KHaargang=2020,
  KHgeoniv="K",
  KHdbname="KHELSAtest20200123.mdb",
  KHlogg="KHlogg.mdb",
...
LagTabellFraFil<-function (filbesk,FGP,batchdate=SettKHBatchDate(),diagnose=0,globs=FinnGlobs(),
                           versjonert=FALSE,echo=FALSE,dumps=list()) {
...
FinnFilgruppeParametre<-function(gruppe,batchdate=SettKHBatchDate(),
  ...
  dbh<-globs$dbh
  datef<-format(strptime(batchdate, "%Y-%m-%d-%H-%M"),"#%Y-%m-%d#")
  ## VERSJONFRA and VERSJONTIL aren't actively used
  ## !OBS! Check if FGP is active via these variabler which aren't used anymore
  ## ---------------------------------------
  ...
  if (FGPfinnes==0){
    ...
    ## HOVEDPARAMETREg is used only to get max age i.e 120 if missing!
    ...
    if(!is.na(FGP$ALDER_ALLE)){
      if (grepl("^\\d+_(\\d+|)$",FGP$ALDER_ALLE)){
        ...
        if (length(alle_aldre)==1){
          ...
          ## This can be deactivated since input from ALDER_ALLE from FILGRUPPER tabel is used
          amin<-as.numeric(alle_aldre[1])
